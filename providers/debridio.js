'use strict';
/*
 * Debridio - Stremio addon designed natively for TorBox / Real-Debrid users.
 * Excellent secondary cache-checker.
 */
const TMDB_API_KEY = '1865f43a0549ca50d341dd9ab8b29f49';
const PROVIDER_NAME = 'Debridio';

const DBR_INSTANCES = [
  'https://debridio.adobotec.com',
  'https://debridio.elfhosted.com',
];
...
async function getStreams(id, type, season, episode) {
  try {
    ...
    let streams = null;
    for (const base of DBR_INSTANCES) {
      try {
        const url = `${base}/stream/${isSeries ? 'series' : 'movie'}/${sid}.json`;
    ...
    const out = [];
    for (const st of streams.slice(0, 20)) {
